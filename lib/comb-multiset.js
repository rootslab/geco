    ...
    return {
        mget : function ( k, v, brep ) {
            let blen = brep.length
                ...
                // get the total number of elements
                , total = -1
                , ok = k && v && brep && brep.length
                ;
            if ( ! ok ) return new RangeError( 'check input values' );

            // choose the correct read fn, only 4, 2, 1 bytes are allowed
            if ( b === 4 ) [ read, write ] = [ 'readUInt32BE', 'writeUInt32BE' ];
            else if ( b === 2 ) [ read, write ] = [ 'readUInt16BE', 'writeUInt16BE' ];
            // else return the 1-byte version, slicing buffer
